...
AtmName='res/B/B0'#_new' # the prefix for all result files related to the set of parameters
#PulsName=AtmName+'P2'
#PulsName='res/B/B0P1'
...
def compf(mass,eqrad,incl_deg,theta_deg,rho_deg,freq,spherical=False):
	...
	if Spectrum=='FromFile' :
		print("reading from file: ", AtmName)
	...
	if computePulse:
		...
		nu=freq #600.0#1.0#100 #600 # star rotation frequency in Hz
		...
		intgspot_version = "sinlgepoint" #"new"
		...
		for p in range(NSpots):
			...
			f=redshift/R*dR
			#print("Flattening:",f, " ",dR," ",flattening)
			#exit()
			...
			for t in range(NPhi):
				if True: # find mu
					...
					if True: # find chi
						...
						tan_chi_prime_sph = beta*cos_alpha*(cos_i*sin_theta-sin_i*cos_theta*cos_phi)/(sin_alpha*sin_psi+beta*sin_i*sin_phi)
						chi_prime_sph=arctan(tan_chi_prime_sph)
